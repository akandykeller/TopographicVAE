...
from tvae.data.mnist import Preprocessor, DualTransformPreprocessor
...
def main():
    ...
    name = 'TVAE_MNIST_Generalization-DualTransform_L=13/36_K=3'

    config['savedir'], config['data_dir'], config['wandb_dir'] = get_dirs()

    savepath = os.path.join(config['savedir'], name)
    preprocessor = Preprocessor(config)
    dual_transform_preprocessor = DualTransformPreprocessor(config)
    train_loader, _, _ = preprocessor.get_dataloaders(batch_size=config['batch_size'])
    _, val_loader, test_loader = dual_transform_preprocessor.get_dataloaders(batch_size=config['batch_size'])
